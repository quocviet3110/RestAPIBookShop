	...
	function loadPayInFindId(id) {
		$.ajax({
			...
			success : function(result) {		
				...
				var str ="";
				$('#date').val(payIn.date);
				$("#status").val(payIn.status).attr('selected','selected');
			 	 $('#idSupplier').val(payIn.supplierPayIn.id).attr('selected','selected'); 
			 	$('#id').val(payIn.id);
			},
	 ...
	 function loadSupplier() {
			$.ajax({
				...
				success : function(result) {		
					...
					document.getElementById('idSupplier').innerHTML = str;
				},
				error : function(error) {
...
<!-- /.main-content -->
						...
						<c:if test="${not empty message}">
							...
							<form class="form-horizontal" role="form" id="formSubmit" enctype="multipart/form-data">
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-2"> Nhà cung cấp: </label>

										<div class="col-sm-9">
												<select  id="idSupplier" name="idSupplier"  data-placeholder="Chọn thể loại sách...">	
												</select>
										</div>
									</div>
									<div class="form-group">
	...
	function addPayIn(data) {
		$.ajax({
			...
			dataType : 'json',
			success : function(result) {
				 window.location.href = "/bookshop/admin/payIn/list";
			},
			error : function(error) {
			}
		});
